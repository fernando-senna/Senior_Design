...
CvScalar COLOR_GREEN = CV_RGB(0, 102, 51);
CvScalar COLOR_BLUE = CV_RGB(0, 0, 255);
CvScalar COLOR_MY_BLUE = CV_RGB(153, 204, 255);
...
//    cv::VideoCapture occulography(cameraIndex);
	cv::VideoCapture occulography("http://129.107.132.26:1234");
    ...
    std::cout<<"height "<< occulography.get(CV_CAP_PROP_FRAME_HEIGHT)  << std::endl;
        ...
        if(occulography.read(eyeImage))
        {
            ...
            if(displayMode)
            {
                ...
                if(trackingSuccess)
                {
                    ...
                    x1 = Center.x - 80; y1 = Center.y - 80; x2 = Center.x + 80; y2 = Center.y + 80;
                    ...
                    cv::Point horizontal_start = cv::Point(x1-100, (y1+y2)/2);
                    cv::Point horizontal_end = cv::Point(x1+260, (y1+y2)/2);
                    
                    // for the vertical line
                    cv::Point vertical_start = cv::Point((x1+x2)/2, y1-100);
                    cv::Point vertical_end = cv::Point((x1+x2)/2, y1+260);
                    
                    // done with the lines

                    cv::ellipse(annotation, tracker.getEllipseRectangle(), COLOR_MY_BLUE, -1);
                    ...
                    cv::rectangle(displayImage, x1y1, x2y2, COLOR_GREEN, 2);
                    // The lines across the rectangle
                    cv::line(displayImage, horizontal_start, horizontal_end, COLOR_WHITE, 1);
                    cv::line(displayImage, vertical_start, vertical_end, COLOR_WHITE, 1);
                    ...
                    cv::putText(displayImage, "Real Time Eye Tracker :: Feel the power of krishna", cv::Point(40, 40), fontFace, 1, COLOR_WHITE, thickness, 8);
                    cv::putText(displayImage, "Wrath of the ONE eye", cv::Point(0, 350), fontFace, 1, COLOR_WHITE, thickness, 8);
