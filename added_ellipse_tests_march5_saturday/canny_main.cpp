...
int main(int argc, char** argv)
{
        ...
        if(occulography.read(eyeImage))
        {
            ...
            if(displayMode)
            {
                ...
                if(trackingSuccess)
                {
                    ...
                    cv::ellipse(displayImage, tracker.getEllipseRectangle(), COLOR_GREEN, 2);
                    // cv::fitEllipse()
                    ...
                    // added a extra circle to try to fit a circle instead of a ellipse
                    //cv::circle(displayImage, Center, 15, COLOR_RED, 2, 4);
                    ...
                    cv::line(displayImage, horizontal_start, horizontal_end, COLOR_RED, 1);
                    cv::line(displayImage, vertical_start, vertical_end, COLOR_RED, 1);
 ...
 //    cv::RotatedRect my_rotated_rect_property;
 //    my_rotated_rect_property = cv::fitEllipse(contours[0]);